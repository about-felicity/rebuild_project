...
3. 用户明确要「短视频 / 图生视频 / 动起来的镜头 / 把这张图做成视频」→ 调用 generate_video_clip（若同条需求须先分镜，见下段「标准成片流程」）。
   ...
   - **一次性（同轮双工具）**：用户已明确要「N 秒成片 / 图生视频」时，你应在**同一轮 assistant 的 tool_use 序列**里依次调用：先 ``generate_storyboard_image``，再 ``generate_video_clip``；**禁止**只生成分镜就收尾。若你漏了第二步，服务端会尝试补跑成片，但你仍应主动连续双调以减少等待。
